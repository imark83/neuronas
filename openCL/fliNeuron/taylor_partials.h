void taylor_partials (real_t x[NCOL]) {
	...
	while (t < TF && !endOfIntegration) {
		fun_partials (t, x, series);		// EVALUATE TAYLOR SERIES
		...
		printf ("\n");*/
		step = getStep_partials (series); 	// GET STEPSIZE
		//printf ("step = %f\n", step);
		...
		horner_partials (NCOL, step, series, x); // HORNER EVALUATION
...
real_t getStep_partials (real_t series[NCOL][ORDER+1]) {
	...
	n1 = normInf_partials (NVAR, x1); n2 = normInf_partials (NVAR, x2);
...
void horner_partials (short n,	 			// NVAR OR NCOL
		real_t h, 			// STEPSIZE
...
real_t normInf_partials (short n,			// NVAR OR NCOL
		real_t *x) {			// ARRAY
