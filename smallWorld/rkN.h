...
void rkN (real_t x[NVAR_N], real_t tf, real_t delay[3*CUTNUMBER], int cutNumber) {
	...
	real_t dense=0;
	real_t dense_step=0.01;
	real_t xDense[NVAR_N];
	real_t fsalBack[NVAR_N];
	...
	while (t<tf && !endOfIntegration) {
		...
		if (fac < 0) {				// rejected step
			...
			if (t+step > dense) {
				...
				rkStepN (xDense, x, dense-t, fsal, eventFlag, eventVal);

				for (j=0; j<NVAR_N; ++j) fsal[j] = fsalBack[j];

				fwrite (&dense, sizeof (real_t), 1, stdout);
				for (j=0; j<20; ++j) fwrite (xDense+3*j, sizeof (real_t), 1, stdout);
				dense += dense_step;
			}
			...
			/*if (t+step > dense) {
				...
				printf ("%e", dense);
				for (j=0; j<2; ++j) printf ("  %e", xDense[3*j]);
				printf ("\n");
				dense += dense_step;
			}*/
			/*if (t+step > dense) {
