...
int main (int argc, char *argv[]) {
	...
	for (k = 21; k<=35; k++) {
		...
		clSetKernelArg (kernel, 0, sizeof (d_VSHIFT), (void*) &d_VSHIFT);
		clSetKernelArg (kernel, 1, sizeof (d_delay), (void*) &d_delay);
		// FOR MEASURING TIME 
		cl_event prof_event;
		...
		char FILENAME[30];
		sprintf (FILENAME, "T%02i.txt", k);


		double P = delay[CUTNUMBER-1]-delay[CUTNUMBER-2];
		double d21 = delay[2*CUTNUMBER-2] - delay[CUTNUMBER-2];
		double d31 = delay[3*CUTNUMBER-2] - delay[CUTNUMBER-2];
		double f21 = d21 / P;
		double f31 = d31 / P;


		printf ("f21 = %.15le\tf31 = %.15le\n", f21, f31);
		if (fabs (f21) < 0.1 || fabs (f31) < 0.1 || fabs (f21 - f31) < 0.1) 
			printf ("VSHIFT = %.15le\tCONVERGE EQUILIBRIO!\n", VSHIFT);
		else if (fabs (f21-1.0) < 0.1 || fabs (f31-1.0) < 0.1) 
			printf ("VSHIFT = %.15le\tCONVERGE EQUILIBRIO!\n", VSHIFT);
		else printf ("VSHIFT = %.15le\tCONVERGE CICLO!\n", VSHIFT);
