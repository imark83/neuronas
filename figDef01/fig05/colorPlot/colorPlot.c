...
int getColor (double* _f21, double* _f31, int N) {
	...
	if (fabs (f21 - 0.3333) + fabs (f31 - 0.6666) < 0.01) return BLACK;

	if (fabs (f21 - 0.6666) + fabs (f31 - 0.3333) < 0.01) return BLACK;
...
int main () {
	...
	fprintf (gnuplot, "set output \"fig05.svg\"\n");
	...
	fprintf (gnuplot, ", \\\n \"./_c10.txt\"  w l lc rgb \"magenta\"");
	fprintf (gnuplot, ", \\\n \"./_c10_.txt\"  w l lc rgb \"magenta\"");
	fprintf (gnuplot, "\n");
