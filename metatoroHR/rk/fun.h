...
void fN (real_t *rop, real_t *x) {
	...
	I_syn[ 0] = GSYN * (gamma[ 1] + gamma[ 2] + interWorld*gamma[ 3] + interWorld*gamma[ 6]);
	I_syn[ 1] = GSYN * (gamma[ 0] + gamma[ 2]);
	I_syn[ 2] = GSYN * (gamma[ 0] + gamma[ 1]);
	I_syn[ 3] = GSYN * (interWorld*gamma[ 0] + gamma[ 4] + gamma[ 5] + interWorld*gamma[ 6]);
	I_syn[ 4] = GSYN * (gamma[ 3] + gamma[ 5]);
	I_syn[ 5] = GSYN * (gamma[ 3] + gamma[ 4]);
	I_syn[ 6] = GSYN * (gamma[ 8] + interWorld*gamma[ 0] + interWorld*gamma[ 3] + gamma[ 7]);
	I_syn[ 7] = GSYN * (gamma[ 8] + gamma[ 6]);
	I_syn[ 8] = GSYN * (gamma[ 6] + gamma[ 7]);

	for (j=0; j<9; j++) {	
		tempRop[0] = x[1+3*j] + x[3*j]*x[3*j] * (B - A*x[3*j]) - x[2+3*j] + IAPP - I_syn[j];
		tempRop[1] = C - D*x[3*j]*x[3*j] - x[1+3*j];
		rop[2+3*j] = R * (S*(x[3*j] - XR) - x[2+3*j]);
		rop[3*j] = tempRop[0];
		rop[1+3*j] = tempRop[1];
