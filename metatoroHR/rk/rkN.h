...
real_t rkStepN (real_t xNext[NVAR_N], real_t x[NVAR_N], real_t h, real_t fsal[NVAR_N], 
		char eventFlag[NNEURON],		// IF NULL INPUT -> NO EVENT SEARCH
						// OUTPUTS EVENT FLAG
		real_t eventVal[NNEURON]) {		// OUTPUTS EVENT TIME value
...
void rkN (real_t x[NVAR_N], real_t tf, __global real_t *delay, int cutNumber) {
	...
	// POINCARE VARIABLES
	char _eventFlag[NNEURON], *eventFlag;
	real_t _eventVal[NNEURON], *eventVal;
	if (delay != (real_t*) 0) {
	...
	int count[NNEURON] = {0, 0, 0, 0, 0, 0, 0, 0, 0};
	...
	while (t<tf && !endOfIntegration) {
		...
		/*************************************/
		//	printf ("%e", t);
		//	for (j=0; j<NNEURON; j++) printf (" %e", x[3*j]);
		//	printf ("\n");
		/*************************************/	
			step = fac * step;
			step = MIN (step, tf-t);
