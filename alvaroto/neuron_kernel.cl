...
__kernel void neuron (__global       real_t *delay,
	...
	rkS (z, 500.0, 0, *vshift);
	// COMPUTE PERIOD FOR SINGLE NEURON
	P = rkS (z, 1000.0, 1, *vshift);
	...
	do {
		...
		rkS (y, (1.0-_phi21)*P, 0, *vshift);
		for (i=0; i<3; i++) x[i+3] = y[i];
	
		for (i=0; i<3; i++) y[i] = x[i];
		rkS (y, (1.0-_phi31)*P, 0, *vshift);
		for (i=0; i<3; i++) x[i+6] = y[i];
	

		rkN (x, 100000.0, T, 3, *vshift);
	...
	// INTEGRATE CPG
	rkN (x, 100000.0, T, CUTNUMBER, *vshift);
